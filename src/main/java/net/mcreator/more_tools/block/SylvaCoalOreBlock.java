...
public class SylvaCoalOreBlock extends MoreToolsElements.ModElement {
	@ObjectHolder("more_tools:sylvacoalore")
	public static final Block block = null;
	public SylvaCoalOreBlock(MoreToolsElements instance) {
		super(instance, 107);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}, new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("sylvacoalore", "sylvacoalore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == StoneSylvaBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 8), Placement.COUNT_RANGE, new CountRangeConfig(12, 1, 1, 128)));
		}
	}
}
