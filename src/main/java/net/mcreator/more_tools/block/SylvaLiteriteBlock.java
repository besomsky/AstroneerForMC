...
public class SylvaLiteriteBlock extends MoreToolsElements.ModElement {
	@ObjectHolder("more_tools:sylvaliterite")
	public static final Block block = null;
	public SylvaLiteriteBlock(MoreToolsElements instance) {
		super(instance, 96);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}, new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("sylvaliterite", "sylvaliterite", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == StoneSylvaBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 6), Placement.COUNT_RANGE, new CountRangeConfig(11, 1, 1, 48)));
		}
	}
}
